...
- [x] 2.2 Remove the `secure: url.protocol === 'https:'` and `httpOnly: true` fields from `Defaults.login`'s returned cookie spec in `plugins/cloudflare-auto-session/src/args.ts`, and verify `yarn build` and `yarn lint` succeed.
